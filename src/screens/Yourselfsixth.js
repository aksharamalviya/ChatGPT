import {
  ...
  const Yourselfsixth = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    const handleSend = async () => { 
        setLoading(true);
        try {
          const prompt =  `write  your introduction for ${textinput} in 1000 words ?`
          //  `${txt1}${txt2}`
  ...
  return (
    <View style={{flex:1}}>
      ...
      <Text style={{marginLeft:35,fontSize:20,color:'#BBA503',fontWeight:'500'}}>Introduce Yourself</Text>
    </TouchableOpacity>
    ...
    <View style={{alignSelf:'flex-start',flexDirection:'row'}}>
      ...
      <Lottie source={require('../assets/Images/yourselflottie2.json')}
     useRef={animationRef}
      autoPlay={true}
     ...
     <View style={{marginLeft:10,marginTop:10,backgroundColor:'#EADD7A',borderRadius:10,paddingLeft:7,padding:2}}>
      <Text style={{}}>Create a introduction idea On  anything !</Text>
     </View>
     </View>
    </View>
    ...
    <TextInput 
      ...
      placeholder='Enter Personal Detail for Introduction...'
      />
  ...
  <TouchableOpacity style={styles.button} onPress={() => handleSend()}>
      ...
      <ScrollView style={{marginHorizontal:10,padding:8,borderRadius:10,backgroundColor:'#EADD7A',height:400}}>
