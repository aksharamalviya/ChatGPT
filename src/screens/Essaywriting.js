import {
  ...
  const Essaywriting = ({navigation}) => {
    ...
    const handleSend = async () => { 
        setLoading(true);
        try {
          const prompt =  `Write Essay for ${textinput} in 1000 words ?`
          //  `${txt1}${txt2}`
  ...
  return (
    ...
    animation="slideInUp" iterationCount={3} direction="alternate">
      ...
      <Text style={{marginLeft:35,fontSize:20,color:'#F58580',fontWeight:'500'}}>Essay Writing</Text>
    </TouchableOpacity>
    ...
    <View style={{alignSelf:'flex-start',flexDirection:'row'}}>
      ...
      <Lottie source={require('../assets/Images/essaylottie2.json')}
     useRef={animationRef}
      autoPlay={true}
      style={{height:60,width:60,marginLeft:3}}
      loop={true}
      />
      <View>
      <Text style={{marginLeft:10}}>With dwellfox</Text>
     <View style={{marginLeft:10,marginTop:10,backgroundColor:'#F5BFBD',borderRadius:10,paddingLeft:7,padding:2}}>
      <Text style={{}}>Create Essay idea On  anything !</Text>
     </View>
     </View>
    </View>
    ...
    <TextInput 
      ...
      placeholder='Enter the Topic for Essay...'
      />
  ...
  <TouchableOpacity style={styles.button} onPress={() => handleSend()}>
      ...
      <ScrollView style={{marginHorizontal:10,padding:8,borderRadius:10,backgroundColor:'#F58580',height:400}}>
